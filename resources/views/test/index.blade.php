	...
	<form action="form" method="POST">
		<p>
			<label for="user">ユーザ名：</label><input type="text" name="user"
				id="user" value="{{ old('user') }}"><br /> @if($errors->has('user'))<span
				class="error">{{ $errors->first('user') }}</span> @endif
		</p>

		<p>
			<label for="mail">メール：</label><input type="text" name="mail"
				id="mail" value="{{ old('mail') }}"><br /> @if($errors->has('mail'))<span
				class="error">{{ $errors->first('mail') }}</span> @endif
		</p>

		<p>
			性別：<input type="radio" name="sex" id="male" value="男性"
				@if(old('sex')=== '男性') checked @endif><label for="male">男性</label>
			<input type="radio" name="sex" id="female" value="女性"
				@if(old('sex')=== '女性') checked @endif><label for="female">女性</label><br />
			@if($errors->has('sex'))<span class="error">{{ $errors->first('sex')
				}}</span> @endif
		</p>
